...
impl ModBuilder {
    ...
    fn build_flash_src(&self) -> Result<()> {
        ...
        build_flash_client_lib(&wg_home)?;
    ...
    fn update_asconfigc_json(&self, flash_sources: &PathBuf) -> Result<()> {
        ...
        let main_class_path = PathBuf::from(&main_class_string_path);

        let prompt_class_string = self.prompt_main_class(main_class_path.to_str().ok_or(Error::ConversionError("main_class_path to string".to_string()))?)?;

        let prompt_class_with_extention = if prompt_class_string.contains(".as") { &prompt_class_string }  else { &format!("{}.as", &prompt_class_string) };
        let prompt_class_globale_path = self.mod_path.join("ui").join("src").join(&prompt_class_with_extention);
        if !prompt_class_globale_path.exists() {
            Err(Error::PathError("Main class path given does not exist".to_string()))?
        }

        let prompt_class_without_extention = if prompt_class_string.contains(".as") { &prompt_class_string.replace(".as", "") }  else { &prompt_class_string };
        ...
        asconfigc.compiler_option.output = PathBuf::from("..").join(&self.build_path).join("res").join("gui").join("flash").join(output).to_string()?;
    ...
    fn prompt_main_class(&self, default: &str) -> Result<String> {
        let value = inquire::Text::new("What's the main ui class path (from ui/src)")
    ...
    fn throw_if_isn_t_mod_folder(&self) -> Result<()> {
        ...
        if meta_path.exists() == false {
            let absolute_mod_folder_path =
                self.mod_path.canonicalize().map_err(|_| Error::PathError("failed to put in canonical form".to_string()))?;
